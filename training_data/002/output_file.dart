...
import 'package:geoflutterfire2/geoflutterfire2.dart';import 'package:firebase_auth/firebase_auth.dart';
...
class EventService {
  ...
  Future<String> createDraftEvent() async {Future<String> createDraftEvent() async {
  ...
  Future<void> saveEvent(Event event) async {Future<void> saveEvent(Event event) async {
  if (event.id.isEmpty) {
  ...
  Stream<Event> getEventStream(String eventId) {Stream<Event> getEventStream(String eventId) {
  final String? uid = _auth.currentUser?.uid;
  if (uid == null) {
    throw Exception('User is not authenticated.');
  }
  ...
  return eventRef.snapshots().map((snapshot) {
    if (snapshot.exists && snapshot.data() != null) {
      ...
      return Event(id: eventId, geoPoint: GeoFirePoint(0, 0));
        ...
        return Event(id: eventId, geoPoint: GeoFirePoint(0, 0)); // Initialize with default GeoFirePoint
      }
    });
  }

  Stream<List<Event>> getEventsNearUser(GeoFirePoint center, double radius) {Stream<List<Event>> getEventsNearUser(GeoFirePoint center, double radius) {
