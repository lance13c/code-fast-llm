...
export function activate(context: vscode.ExtensionContext, clipboard: { readText: () => Thenable<string | undefined> } = vscode.env.clipboard) {
  console.log("active");

  if (!commandDisposable) {
    const applyCodeBlocksCommand = vscode.commands.registerCommand('code-fast-llm.applyCodeBlocks', async () => {
      ...
      if (!idPattern.test(first40Chars)) {
        console.log('Clipboard text does not contain "id":"cfllm" within the first 40 characters. Performing normal paste.');
      ...
      const findMarkerPosition = (marker: string, after: boolean = true): number => {
      ...
      codeResponse.codeBlocks.forEach(block => {
        ...
        if (block.type === 'add') {
          ...
          console.log(`Replacing code from position ${startPosition} to ${endPosition}`);
          edit.replace(document.uri, new vscode.Range(start, end), block.code || '');
        } else if (block.type === 'delete') {
          console.log(`Deleting code from position ${startPosition} to ${endPosition}`);
      ...
      vscode.workspace.applyEdit(edit).then(success => {
        ...
        if (success) {
          vscode.window.showInformationMessage('Code blocks applied successfully');
        } else {
